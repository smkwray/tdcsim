...
1. **TIPS inflation accretion excluded from financing cost.** When `cpi_annual_inflation > 0`, TIPS principal grows via index-ratio adjustment, but this accretion is not recorded in `FinancingCost_Period`. The financing cost identity holds on cash flows only. This is appropriate for the shipped zero-inflation configuration but would understate financing cost in positive-inflation scenarios.
...
3. **Initial portfolio is stylized, not config-derived.** The portfolio generator (`csv_gen.py`) uses hard-coded maturity menus, coupon ranges, and TIPS reference CPI values that are independent of the scenario's yield curve and issuance profile. This means the starting debt stock is a representative approximation, not an exact reflection of the model's own issuance parameters.
